...
class HTMLRewriter {
    ...
    process(source, config = {}) {
        ...
        iterate(ast, node => {
            ...
            switch(node.tagName) {
                case 'STYLE':
                    if (node.textContent) node.textContent = this.ctx.css.process(node.textContent, config);
                    break;
                case 'TITLE':
            ...
            node.attrs.forEach(attr => {
                ...
                switch(handler) {
                    case 'url':
                        ...
                        attr.value = this.ctx.url.wrap(attr.value, { ...config, flags });
                        break;
                    case 'srcset':
                        node.setAttribute(`corrosion-${attr.name}`, attr.value);
                        attr.value = this.srcset(attr.value, config);
                        break;
                    case 'css':
                        attr.value = this.ctx.css.process(attr.value, { ...config, context: 'declarationList' });
                        break;
                    case 'html':
                        node.setAttribute(`corrosion-${attr.name}`, attr.value);
                        attr.value = this.process(attr.value, meta);
                        break;
                    case 'delete':
        ...
        if (config.document) {
            for (let i in ast.childNodes) if (ast.childNodes[i].tagName == 'html') ast.childNodes[i].childNodes.forEach(node => {
                if (node.tagName == 'head') node.childNodes.unshift(...this.injectHead(config.base));
            });
        };
