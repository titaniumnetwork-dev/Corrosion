...
function createRequestProxy(ctx) {
    return async function onRequest(clientRequest, clientResponse) {
        try {
            ...
            if (clientRequest.url.split('?')[0].startsWith(`${ctx.prefix}index.js`)) return fs.existsSync(path.join(__dirname, '../../bundle.js')) ? ( 
                clientResponse.setHeader('Content-Type', 'application/javascript'),
                clientResponse.end(fs.readFileSync(path.join(__dirname, '../../bundle.js'), 'utf-8'))
            ) : clientResponse.end('Script is still compiling or does not exist.');
            const urlData = ctx.url.unwrap(clientRequest.url, { flags: true, leftovers: true, });
            const requestContext = {
                url: new URL(urlData.value),
                flags: urlData.flags,
